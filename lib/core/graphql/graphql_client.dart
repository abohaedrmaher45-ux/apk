...
class LoggingHttpClient extends http.BaseClient {
  ...
  /// When false, X-CHANNEL / X-LOCALE / X-CURRENCY are NOT attached.
  ///
  /// The cart endpoint needs this: the working Postman `cartDetail` request
  /// sends ONLY `is-cookie-exist: 0` + `Authorization: Bearer <token>`, and
  /// sending extra store-context headers (an X-CURRENCY the channel has no
  /// exchange rate for, in particular) makes Bagisto throw a 500 with an
  /// EMPTY graphqlErrors list while formatting prices.
  final bool attachDynamicHeaders;

  LoggingHttpClient({
    required http.Client inner,
    this.attachDynamicHeaders = true,
  }) : _inner = inner;
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    try {
      ...
      _logApiMessage('📦 Response Length: ${responseBody.length} chars');
      _logApiMessage('═══════════════════════════════════════════');
  ...
  Future<void> _attachDynamicHeaders(http.BaseRequest request) async {
    if (!attachDynamicHeaders) return;
    ...
    request.headers['X-CURRENCY'] = 'EUR';
  }
...
class GraphQLClientProvider {
  ...
  static GraphQLClient buildClient({String? token}) {
    ...
    final httpLink = HttpLink(
      ...
      defaultHeaders: {
        ...
        // Resolve the cart by the Bearer customer identity instead of a
        // browser cookie session (mobile has no cookies). Without this the
        // storefront returns a null/empty cart even though the DB row exists.
        // Matches the working Postman "Cart" request header.
        'is-cookie-exist': '0',
      },
    );
  ...
  /// Client for CART requests ONLY — mirrors the working Postman `cartDetail`
  /// request byte-for-byte in terms of headers.
  ///
  /// Postman sends exactly:
  ///   is-cookie-exist: 0
  ///   Authorization: Bearer <customerToken>
  ///
  /// It does NOT send X-STOREFRONT-KEY, X-CHANNEL, X-LOCALE or X-CURRENCY.
  /// Sending those made cartDetail return HTTP 500 with an EMPTY graphqlErrors
  /// list (a PHP-level crash, not a schema rejection) even though the very same
  /// query — with MORE fields — succeeded in Postman.
  ///
  /// If you need store context on the cart later, re-add ONE header at a time
  /// and re-test; do not re-add them in bulk.
  static GraphQLClient buildCartClient({String? token}) {
    final httpClient = LoggingHttpClient(
      inner: TimeoutHttpClient(
        connectTimeout: _kTimeout,
        receiveTimeout: _kTimeout,
      ),
      // Critical: suppress X-CHANNEL / X-LOCALE / X-CURRENCY.
      attachDynamicHeaders: false,
    );
    ...
    if (token != null && token.isNotEmpty) {
      ...
      link = authLink.concat(
        _createLoggingLink(label: 'Cart').concat(httpLink),
      );
    } else {
      link = _createLoggingLink(label: 'Cart').concat(httpLink);
    }
  ...
  static ValueNotifier<GraphQLClient> get client {
    ...
    // Attach the stored customer token (if the user is logged in) to every
    // request, so storefront queries that require auth (e.g. product detail
    // via allProducts) succeed instead of returning "Unauthenticated".
    ...
    final link = authLink.concat(_createLoggingLink().concat(httpLink));
  ...
  static ValueNotifier<GraphQLClient> authenticatedClient(String accessToken) {
    ...
    // Read the token FRESH on every request instead of baking in the string
    // captured when this client was built.
    //
    // ─── WHY ────────────────────────────────────────────────────────────
    // This used to be `getToken: () async => 'Bearer $accessToken'`, where
    // `accessToken` is a closure capture of whatever the token was at
    // CONSTRUCTION time. `AccountRepository` is built once from
    // `authClient.value` (main.dart, account_page.dart, checkout_page.dart …)
    // and then kept, so once the token was renewed — or when the client was
    // built before login completed — every later request kept sending the STALE
    // token. The server rejected it, and Bagisto surfaces that as a 500 rather
    // than a clean 401, which is exactly the "500 Internal Error" that appeared
    // on the cart, the saved addresses and checkout, and which "logging in
    // again" fixed: re-login rebuilt the client with a fresh capture.
    //
    // Reading from AuthStorage's key on each call means a renewed token is
    // picked up immediately, with no need to rebuild clients or re-login.
    // (SharedPreferences key duplicated here rather than importing
    // AuthStorage, which would create a core -> features circular import.)
    final authLink = AuthLink(
      getToken: () async {
        try {
          final prefs = await SharedPreferences.getInstance();
          final current = prefs.getString('auth_token');

          if (current != null && current.isNotEmpty) {
            return 'Bearer $current';
          }
        } catch (e) {
          debugPrint('[GraphQL] could not read stored token: $e');
        }

        // Fall back to the token this client was created with (e.g. a flow that
        // passes a token explicitly before it has been persisted).
        return accessToken.isEmpty ? null : 'Bearer $accessToken';
      },
    );
