...
class FCMService {
  ...
  Future<String?> _getAndSaveDeviceToken() async {
    try {
      ...
      if (token != null && token.isNotEmpty) {
        ...
        // Register with the backend immediately if the user is already
        // logged in (has a Sanctum token) — no-op otherwise.
        unawaited(FcmRegistrationService.syncIfPossible());
        return token;
      } else {
  ...
  Future<void> _handleTokenRefresh(String newToken) async {
    try {
      ...
      // Per the integration doc: re-send immediately on every refresh.
      // No-op if the user isn't logged in yet (no Sanctum token stored).
