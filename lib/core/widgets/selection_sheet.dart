...
class SelectionSheet<T> extends StatefulWidget {
  ...
  /// Optional live data. When the list is still loading, the caller can open
  /// the sheet immediately and stream results in — so the user taps ONCE
  /// instead of once to load and again to pick.
  ///
  /// All of these are optional; existing call sites that pass a ready-made
  /// [items] list keep working unchanged.
...
class _SelectionSheetState<T> extends State<SelectionSheet<T>> {
  ...
  StreamSubscription<List<T>>? _itemsSub;
  ...
  void initState() {
    ...
    // Assume "loading" when we open with nothing but expect a stream to fill
    // us in — otherwise the sheet would flash "no results" for a moment.
    _isLoading = widget.items.isEmpty && widget.itemsStream != null;
    ...
    _loadingSub = widget.isLoadingStream?.listen((loading) {
      if (!mounted) return;
      setState(() => _isLoading = loading);
    });

    _errorSub = widget.errorStream?.listen((error) {
      if (!mounted) return;
      setState(() {
        _error = error;
          ...
          children: [
            ...
            Flexible(
              child: _isLoading && _items.isEmpty
                  // Loading: show a real progress indicator instead of an empty
                  // list, so the sheet can be opened on the FIRST tap.
                  ? const Center(
                      child: Padding(
