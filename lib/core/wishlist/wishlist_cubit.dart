...
class WishlistCubitState extends Equatable {
  ...
  bool isWishlisted(int productId) => wishlistedProducts.containsKey(productId);
...
class WishlistCubit extends Cubit<WishlistCubitState> {
  ...
  Future<void> loadWishlist() async {
    try {
      ...
      debugPrint('❤️ WishlistCubit: loaded ${map.length} wishlisted products');
      emit(WishlistCubitState(wishlistedProducts: map, isLoaded: true));
    } catch (e) {
  ...
  Future<bool?> toggleWishlist({required int productId}) async {
    ...
    // Mark as processing
    emit(state.copyWith(processingIds: {...state.processingIds, productId}));
    ...
    try {
      ...
      if (state.isWishlisted(productId)) {
        ...
        // Bagisto removes by PRODUCT id, not by the wishlist row id.
        debugPrint('❤️ WishlistCubit: removing product $productId');

        await repo.removeFromWishlist(productId: productId);
