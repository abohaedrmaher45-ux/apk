...
class WishlistBloc extends Bloc<WishlistEvent, WishlistState> {
  ...
  Future<void> _onRemove(
    ...
    try {
      // Resolve the PRODUCT id BEFORE calling the API: Bagisto's
      // `removeFromWishlist` is keyed by product id, not by the wishlist row.
      //
      // Note the previous `orElse: () => state.items.first` fallback was unsafe
      // — if the row was missing it silently targeted an UNRELATED product.
      // Return null and abort instead.
      ...
      if (productId == null) {
        debugPrint('❌ Wishlist remove: no productId for row ${event.id}');
        emit(
          state.copyWith(
            processingIds: Set<String>.from(state.processingIds)
              ..remove(event.id),
