  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<OrderDetailBloc, OrderDetailState>(
        ...
        builder: (context, state) {
          // PREVIEW-ONLY: show a fully-populated mock order detail when the
          // backend isn't connected. Real data always takes priority.
          if (kUsePreviewData && state.order == null) {
            return _OrderDetailBody(
              order: PreviewData.orderDetail(
                incrementId: orderNumber ?? '1257',
              ),
              orderId: orderId,
            );
          }
...
class _OrderDetailBodyState extends State<_OrderDetailBody> {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Order tracking timeline (visual, derived from status)
  ...
  // A visual stepper derived from the order status. Display-only: it reads the
  // existing status string and does not change any logic.
  Widget _buildTrackingTimeline(bool isDark) {
    final l10n = AppLocalizations.of(context)!;

    // Resolve raw status → one of the 6 customer-facing states.
    final displayStatus = OrderStatusMapper.resolve(
      widget.order.status,
      hasShipment: widget.order.shipments.isNotEmpty,
    );
    final canceled = OrderStatusMapper.isCanceled(displayStatus);
    final currentStep = OrderStatusMapper.timelineIndex(displayStatus);

    // Steps: New → Preparing → Ready → Out for delivery → Delivered
    final steps = <_TrackStep>[
      _TrackStep(l10n.orderStatusNew, Icons.receipt_long_outlined),
      _TrackStep(l10n.orderStatusPreparing, Icons.inventory_2_outlined),
      _TrackStep(l10n.orderStatusReady, Icons.shopping_bag_outlined),
      _TrackStep(
        l10n.orderStatusOutForDelivery,
        Icons.local_shipping_outlined,
      ),
      _TrackStep(l10n.orderStatusDelivered, Icons.check_circle_outline),
    ];
    ...
    if (canceled) {
      return Container(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFFF7E9F2),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: const Color(0xFFE6C2D8)),
        ),
        child: Row(
          children: [
            const Icon(
              Icons.cancel_outlined,
              color: Color(0xFFB13E7E),
              size: 22,
            ),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
                isAr ? 'تم إلغاء هذا الطلب.' : 'This order has been canceled.',
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: SentryPalette.softViolet.withValues(alpha: 0.4),
        ),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: List.generate(steps.length * 2 - 1, (i) {
          if (i.isOdd) {
            // Connector line between two nodes. Positioned at the vertical
            // center of the 36px circles (top-aligned row), so it lines up
            // regardless of how many lines the labels wrap to.
            final leftIndex = i ~/ 2;
            final done = leftIndex < currentStep;
            return Expanded(
              child: Container(
                height: 3,
                margin: const EdgeInsets.only(top: 16),
                color: done
                    ? SentryPalette.violet
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: done ? SentryPalette.brandGradient : null,
                    color: done
                        ? null
                        : SentryPalette.softViolet.withValues(alpha: 0.25),
                    boxShadow: isCurrent
                        ? [
                            BoxShadow(
                              color:
                                  SentryPalette.violet.withValues(alpha: 0.35),
                              blurRadius: 10,
                  ...
                  child: Icon(
                    ...
                    color: done
                        ? Colors.white
                        : SentryPalette.violet.withValues(alpha: 0.7),
                  ),
                ),
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      color: done
                          ? SentryPalette.ink
                          : SentryPalette.ink.withValues(alpha: 0.5),
                    ),
                  ),
                ),
              ],
  ...
  Widget _buildStatusRow(bool isDark) {
    ...
    // Resolve the raw Bagisto status into one of the 6 customer-facing states.
    final displayStatus = OrderStatusMapper.resolve(
      order.status,
      hasShipment: order.shipments.isNotEmpty,
    );
    final chipColors = _getStatusColors(displayStatus);
    ...
    return Wrap(
      ...
      children: [
        ...
        Container(
          ...
          child: Text(
            OrderStatusMapper.label(displayStatus, l10n),
            style: TextStyle(
              fontFamily: 'Roboto',
  ...
  /// Status chip colours — varied violet shades (Sentry palette).
  static _StatusChipColors _getStatusColors(OrderDisplayStatus status) {
    switch (status) {
      case OrderDisplayStatus.newOrder:
        return const _StatusChipColors(
          bg: Color(0xFFE9F0FF),
          border: Color(0xFFC9DBFF),
          text: Color(0xFF3B6FE0),
        );
      case OrderDisplayStatus.preparing:
        return const _StatusChipColors(
          bg: Color(0xFFF1EAFE),
          border: Color(0xFFD6B4FE),
          text: Color(0xFF7C4DD6),
        );
      case OrderDisplayStatus.ready:
        return const _StatusChipColors(
          bg: Color(0xFFFFF3E0),
          border: Color(0xFFFFD9A0),
          text: Color(0xFFC77700),
        );
      case OrderDisplayStatus.outForDelivery:
        return const _StatusChipColors(
          bg: Color(0xFFE7F3FF),
          border: Color(0xFFB6DBF8),
          text: Color(0xFF1F76C4),
        );
      case OrderDisplayStatus.delivered:
        return const _StatusChipColors(
          bg: Color(0xFFE6F6EC),
          border: Color(0xFFBDE5CB),
          text: Color(0xFF2E9E5B),
        );
      case OrderDisplayStatus.canceled:
        return const _StatusChipColors(
          bg: Color(0xFFF7E9F2),
          border: Color(0xFFE6C2D8),
          text: Color(0xFFB13E7E),
        );
    }
  }
}
...
/// A single step in the order tracking timeline (display-only).
class _TrackStep {
  final String label;
  final IconData icon;
  const _TrackStep(this.label, this.icon);
}
