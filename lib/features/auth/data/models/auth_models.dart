...
class CustomerLogin {
  ...
  factory CustomerLogin.fromJson(Map<String, dynamic> json) {
    // الباك إند (Bagisto) يعيد توكن JWT في الحقل `accessToken` على المستوى
    // الأعلى. نقرأه أولًا، ثم نلجأ إلى بدائل قديمة للتوافق فقط.
    ...
    return CustomerLogin(
      ...
      apiToken:
          accessToken ??
          json['apiToken']?.toString() ??
          customer?['apiToken']?.toString(),
      token:
          accessToken ??
          json['token']?.toString() ??
          customer?['token']?.toString(),
