...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  /// Fetches the Sanctum token (via `updateAccount`) and, once available,
  /// registers the FCM device token with the backend's REST endpoint — the
  /// two-step flow from the push-notifications integration doc. Fire-and-
  /// forget: never awaited by callers and never throws, so it can't delay
  /// or break login/registration/session-restore.
  Future<void> _syncPushNotificationToken() async {
    try {
      final sanctumToken = await repository.fetchSanctumToken();
      if (sanctumToken == null || sanctumToken.isEmpty) return;
      await SanctumTokenService.saveToken(sanctumToken);
      await FcmRegistrationService.syncIfPossible();
    } catch (e) {
      debugPrint('⚠️ Push notification token sync failed (non-fatal): $e');
    }
  }
    ...
    if (token != null && token.isNotEmpty) {
      try {
        ...
        if (isValid) {
          ...
          unawaited(_syncPushNotificationToken());
        } else {
          debugPrint('🔐 AuthBloc._onCheckStatus: token is invalid/expired, clearing session');
          await AuthStorage.clearAuth();
          emit(const AuthUnauthenticated());
        }
      } catch (e) {
        debugPrint('🔐 AuthBloc._onCheckStatus: transient validation error ($e), preserving offline session');
        final name = await AuthStorage.getUserName();
        final email = await AuthStorage.getUserEmail();
        final userId = await AuthStorage.getUserId();
        final deviceToken = await DeviceTokenService.getDeviceToken();
  ...
  Future<void> _onLogout(
    ...
    // Clear local authentication data and GraphQL cache immediately
    await AuthStorage.clearAuth();
    await GraphQLClientProvider.clearCache();

    // Trigger repository logout in background without waiting for network response
    repository.logout().catchError((e) {
      debugPrint('Background logout API error: $e');
      return false;
    });

    debugPrint('✅ Logged out immediately');
    emit(const AuthUnauthenticated());
  }
}
