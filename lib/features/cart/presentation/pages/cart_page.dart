...
class _CartPageState extends State<CartPage> {
  ...
  Widget _buildBody(BuildContext context, CartState state, bool isDark) {
    if (state.status == CartStatus.loading) {
      return const Center(
        child: CircularProgressIndicator(color: SentryPalette.violet),
      );
    }
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                CurrencyFormatter.normalizeDigits(
  ...
  Widget _buildPriceRow(
    ...
    // Backend-formatted totals arrive with Arabic-Indic digits when the store
    // locale is Arabic (e.g. "\u200f\u0664\u0665\u066b\u0660\u0660 \u20ac"). The item rows already go
    // through CurrencyFormatter via displayPrice/displayTotal, but these
    // Price-Break rows read `cart.formatted*` directly — which is exactly why
    // the wrong digits showed up "in the totals only".
    final value = CurrencyFormatter.normalizeDigits(rawValue);
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(
              AppLocalizations.of(context)!.cartRemove,
              style: const TextStyle(color: SentryPalette.violet),
            ),
          ),
        ],
      ),
