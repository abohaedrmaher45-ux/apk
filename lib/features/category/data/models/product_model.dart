...
class ProductModel {
  ...
  /// ISO currency code from `priceHtml.currencyCode` (e.g. "EUR").
  /// Carried so screens that re-format the RAW price (rather than reusing the
  /// backend's formatted string) still show the correct symbol.
  ...
  String get formattedDisplayPrice {
    ...
    } else if (formattedPrice?.isNotEmpty ?? false) {
      label = formattedPrice!;
    } else {
      // Fallback: format the raw price with THIS PRODUCT's currency code, so
      // the symbol is right even before the channel bootstrap has completed.
  ...
  /// Product weight / volume (الوزن أو الحجم).
  /// Prefers the direct [weightValue] field (used by preview/mock data and
  /// any API field that maps to it); otherwise reads from the already-fetched
  /// attributeValues list by attribute code. Matches Bagisto's default
  /// `weight` attribute; also supports a custom `volume` attribute.
  String? get weight {
    if (weightValue != null && weightValue!.trim().isNotEmpty) {
      return weightValue!.trim();
    }
    for (final attr in attributeValues) {
      final code = attr.attributeCode?.toLowerCase();
      if ((code == 'weight' || code == 'volume') &&
  ...
  /// All image URLs (prefers the backend's full `url`, then path fallbacks)
  List<String> get allImageUrls {
    if (images.isNotEmpty) {
      return images.map((img) => img.url).where((u) => u.isNotEmpty).toList();
    }
  ...
  /// Convenience: first usable image URL, or null.
  ...
  factory ProductModel.fromJson(Map<String, dynamic> json) {
    ...
    // ── Derive fields that the real Bagisto Product type doesn't expose
    // directly (baseImageUrl, formattedPrice, specialPrice, ...) ──
    final parsedImages = _parseImages(json['images']);
    String? derivedBaseImage = json['baseImageUrl'] as String?;
    if ((derivedBaseImage == null || derivedBaseImage.isEmpty) &&
        parsedImages.isNotEmpty) {
      derivedBaseImage = parsedImages.first.url;
    }

    final priceHtml = json['priceHtml'] is Map<String, dynamic>
        ? json['priceHtml'] as Map<String, dynamic>
        : const <String, dynamic>{};

    // Regular (list) price
    final derivedFormattedPrice =
        (json['formattedPrice'] as String?) ??
        priceHtml['formattedRegularPrice'] as String? ??
        priceHtml['formattedFinalPrice'] as String?;

    // Special price: prefer explicit specialPrice, else finalPrice from priceHtml
    dynamic rawSpecial = json['specialPrice'];
    if (rawSpecial == null && priceHtml.isNotEmpty) {
      final finalP = priceHtml['finalPrice'];
      final regularP = priceHtml['regularPrice'];
      // Only treat finalPrice as a "special" when it is lower than regular.
      final fp = _parseDouble(finalP);
      final rp = _parseDouble(regularP);
      if (fp != null && rp != null && fp < rp) rawSpecial = finalP;
    }
    final derivedFormattedSpecial =
        (json['formattedSpecialPrice'] as String?) ??
        priceHtml['formattedFinalPrice'] as String?;

    // Custom select attributes (brand, color, size, …) are NOT top-level
    // fields on simple products — Bagisto returns them inside `additionalData`
    // (or legacy `attributeValues`) as { code, label, value }. Extract by code.
    String? attrByCode(String wanted) {
      final lists = <dynamic>[json['additionalData'], json['attributeValues']];
      for (final list in lists) {
        if (list is List) {
          for (final entry in list.whereType<Map<String, dynamic>>()) {
            final code =
                (entry['code'] ??
                        (entry['attribute'] is Map
                            ? entry['attribute']['code']
                            : null))
                    ?.toString()
                    .toLowerCase();
            if (code == wanted) {
              final v = (entry['value'] ?? entry['textValue'])?.toString();
              // `value` may legitimately be null (e.g. brand not set) → skip.
    ...
    final derivedBrand = (json['brand'] as String?)?.trim().isNotEmpty == true
        ? (json['brand'] as String).trim()
        : attrByCode('brand');
    final derivedColor = (json['color'] as String?)?.trim().isNotEmpty == true
        ? (json['color'] as String).trim()
        : attrByCode('color');
    final derivedSize = (json['size'] as String?)?.trim().isNotEmpty == true
        ? (json['size'] as String).trim()
        : attrByCode('size');
    ...
    return ProductModel(
      ...
      formattedPrice: derivedFormattedPrice,
      currencyCode:
          (json['currencyCode'] as String?) ??
          priceHtml['currencyCode'] as String?,
      baseImageUrl: derivedBaseImage,
      minimumPrice: _parseDouble(json['minimumPrice']),
      formattedMinimumPrice: json['formattedMinimumPrice'] as String?,
      specialPrice: _parseSpecialPrice(rawSpecial),
      formattedSpecialPrice: derivedFormattedSpecial,
      isSaleable: _parseBool(json['isSaleable']),
      color: derivedColor,
      size: derivedSize,
      brand: derivedBrand,
      weightValue: json['weight']?.toString(),
      ...
      // Merge both attribute sources: the detail query returns custom
      // attributes (brand, weight, volume…) under `additionalData`, while other
      // queries use `attributeValues`. Combine so getters find them either way.
      attributeValues: [
        ..._parseAttributeValues(json['attributeValues']),
        ..._parseAttributeValues(json['additionalData']),
      ],
  ...
  static List<ProductReview> _parseReviews(dynamic json) {
    if (json == null) return [];
    // Real Bagisto returns a plain list; demo schema used { edges: [{node}] }.
    if (json is List) {
      return json
  ...
  static List<AttributeValueItem> _parseAttributeValues(dynamic json) {
    if (json == null) return [];
    // New storefront shape: a plain list (from `additionalData`).
    if (json is List) {
      return json
          .whereType<Map<String, dynamic>>()
...
class ProductImage {
  ...
  String get url => fullUrl ?? publicPath ?? path;
...
class AttributeValueItem {
  ...
  factory AttributeValueItem.fromJson(Map<String, dynamic> json) {
    // New `additionalData` shape: { code, label, value, admin_name }
    if (json.containsKey('code') || json.containsKey('admin_name')) {
      return AttributeValueItem(
        value: json['value']?.toString(),
        attributeCode: json['code']?.toString(),
        attributeAdminName: (json['admin_name'] ?? json['label'])?.toString(),
      );
    }
    // Legacy shape: { value, attribute: { code, adminName } }
...
class PaginatedProducts {
  ...
  factory PaginatedProducts.fromJson(Map<String, dynamic> json) {
    // Real Bagisto: allProducts { paginatorInfo { ... }, data [ ... ] }
    final connection =
        (json['allProducts'] ?? json['products']) as Map<String, dynamic>?;
    if (connection == null) {
    ...
    if (dataList != null) {
      ...
      final total = info?['total'] as int? ?? dataList.length;
      ...
      return PaginatedProducts(
        ...
        pageInfo: PageInfo(
          ...
          // Encode the NEXT page number as the cursor so paginated callers
          // that pass `after` keep working unchanged.
