...
class LoadProductList extends ProductListEvent {
  ...
  /// Sort to apply from the very first load (e.g. coming from the seasonal
  /// banner button, which always opens "newest products"). Null keeps the
  /// bloc's default sort.
  final SortOption? initialSort;

  /// When true, only products with an active discount are kept (client-side,
  /// same approach as the existing price-range filter — the backend has no
  /// "on sale" filter). Used by the offers-banner button.
...
class ProductListState extends Equatable {
  ...
  /// When true, every fetched page is filtered client-side to only products
  /// with an active discount (set once from [LoadProductList.discountedOnly]
  /// and preserved across sort/filter/pagination for the lifetime of this page).
  final bool discountedOnly;
...
class ProductListBloc extends Bloc<ProductListEvent, ProductListState> {
  ...
  Future<void> _onLoadProductList(
    ...
    // One-time overrides coming from banner navigation (e.g. the seasonal
    // banner forces "newest" sort, the offers banner forces discounted-only).
    // A fresh CategoryProductsGridPage always creates a brand-new bloc, so
    // there's no prior state to worry about clobbering here.
    final effectiveSort = event.initialSort ?? state.currentSort;
    ...
    try {
      final cachedResult = await repository.getFilterProducts(
        filter: filterString,
        first: 12,
        sortKey: effectiveSort.sortKey,
        reverse: effectiveSort.reverse,
  ...
  /// Applies filters the backend has no equivalent for, so they're done
  /// client-side on whatever page was just fetched: price range (existing)
  /// and "discounted only" (offers-banner landing page).
  List<ProductModel> _applyClientFilters(List<ProductModel> products) {
    var result = products;
    if (state.selectedPriceMax != null || state.selectedPriceMin != null) {
      final minVal = state.selectedPriceMin ?? 0.0;
      final maxVal = state.selectedPriceMax ?? 999999.0;
      result = result
          .where((p) => p.displayPrice >= minVal && p.displayPrice <= maxVal)
          .toList();
    }
    if (state.discountedOnly) {
      result = result.where((p) => p.discountPercent != null).toList();
    }
    return result;
  }
  ...
  Future<void> _fetchFromNetwork(
    ...
    try {
      ...
      var productsList = _applyClientFilters(result.products);
  ...
  Future<void> _refreshFromNetwork(
    ...
    try {
      ...
      if (result.products.isNotEmpty) {
        final productsList = _applyClientFilters(result.products);
  ...
  Future<void> _onLoadMore(
    ...
    try {
      ...
      final newProducts = _applyClientFilters(result.products);

      debugPrint(
        '[ProductListBloc] LoadMore got ${newProducts.length} more (of ${result.products.length} fetched), '
        'total now=${state.products.length + newProducts.length}',
      );
