...
class CategoryProductsGridPage extends StatelessWidget {
  ...
  /// Sort applied from the first load (e.g. "newest" when opened from the
  /// seasonal banner button). Null keeps the bloc's default sort.
  final SortOption? initialSort;

  /// When true, only products with an active discount are shown (used when
  /// opened from the offers-banner button).
...
class _CategoryProductsGridViewState extends State<_CategoryProductsGridView>
  ...
  Widget _buildErrorState(BuildContext context, String? errorMessage) {
    final l10n = AppLocalizations.of(context)!;
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final isNetwork = errorMessage == l10n.commonUnableToReachServer ||
        errorMessage == l10n.commonNetworkError ||
        errorMessage == l10n.commonTimeoutError;
