...
class BagistoCountry {
  ...
  /// Pick the localized name for [locale] from a Bagisto `translations` list,
  /// falling back to the default name.
  ///
  /// Bagisto matches on the locale CODE (e.g. "ar"), and translation rows look
  /// like `{ locale: "ar", name: "\u0623\u0644\u0645\u0627\u0646\u064a\u0627" }`. We also accept a
  /// language-only match so "ar_SA" still resolves against an "ar" row.
  static String? localizedFrom(
    dynamic translations,
    String? locale,
    String key,
  ) {
    if (locale == null || locale.isEmpty || translations is! List) return null;
    ...
    for (final row in translations.whereType<Map<String, dynamic>>()) {
      final rowLocale = row['locale']?.toString();
      if (rowLocale == null) continue;

      final value = row[key]?.toString();
      if (value == null || value.trim().isEmpty) continue;

      if (rowLocale.toLowerCase() == locale.toLowerCase()) {
        exact = value;
        break;
      }
      if (rowLocale.split(RegExp(r'[_-]')).first.toLowerCase() == lang) {
        byLang ??= value;
      }
    }
  ...
  /// [locale] — the app's current locale code, used to resolve the localized
  /// country name from the `translations` relation.
  factory BagistoCountry.fromJson(Map<String, dynamic> json, {String? locale}) {
    // This schema has NO `_id` field: `Country.id` IS the numeric id (it just
    // arrives as a GraphQL `ID!`, i.e. a string like "82"). Reading `_id` here
    // always yielded 0, which meant even a hand-picked country produced
    // countryId=0 and the states lookup could never run.
    final rawId = json['id']?.toString() ?? '';

    final translated = localizedFrom(json['translations'], locale, 'name');

    return BagistoCountry(
      id: rawId,
      numericId: int.tryParse(rawId) ?? 0,
...
class BagistoCountryState {
  ...
  factory BagistoCountryState.fromJson(
    Map<String, dynamic> json, {
    String? locale,
  }) {
    // Same as Country: `id` is the numeric id; there is no `_id`.
    final rawId = json['id']?.toString() ?? '';
    ...
    return BagistoCountryState(
      ...
      numericId: int.tryParse(rawId) ?? 0,
      code: json['code'] as String?,
      defaultName: translated ?? json['defaultName'] as String? ?? '',
...
class ShippingRate {
  ...
  factory ShippingRate.fromJson(Map<String, dynamic> json) {
    return ShippingRate(
      ...
      baseFormattedPrice: CurrencyFormatter.normalizeDigits(
        json['baseFormattedPrice'] as String?,
      ),
  ...
  String get displayPrice {
    final raw = formattedPrice ?? CurrencyFormatter.formatAmount(price);
    return CurrencyFormatter.normalizeDigits(raw);
  }
