...
class CheckoutState extends Equatable {
  ...
  /// True while the countries list is being fetched.
  ///
  /// The UI used to derive this from `countries.isEmpty`, which meant a FAILED
  /// fetch was indistinguishable from an in-flight one — the country field then
  /// spun forever and could never be edited. Track it explicitly instead.
  final bool countriesLoading;

  /// Error text from the last countries fetch, or null on success.
  /// Surfaced in the UI so a failed load is visible instead of silently
  /// leaving the country field empty and un-tappable.
  ...
  CheckoutState copyWith({
    ...
    return CheckoutState(
      ...
      countriesError: clearCountriesError
          ? null
...
class CheckoutBloc extends Bloc<CheckoutEvent, CheckoutState> {
  ...
  Future<void> _onFetchCountries(
    ...
    // Don't re-fetch if already loaded, or if a fetch is already in flight.
    if (state.countries.isNotEmpty || state.countriesLoading) return;

    emit(state.copyWith(countriesLoading: true, clearCountriesError: true));
    ...
    try {
      ...
      if (countries.isEmpty) {
        // A successful request that returns nothing is still a failure from the
        // user's point of view — record it so the field can say so.
        debugPrint('[CheckoutBloc] fetchCountries returned an EMPTY list');
        emit(
          state.copyWith(
            countriesLoading: false,
            countriesError: 'No countries returned by the server',
          ),
        );
        return;
      }
      ...
      // CRITICAL: always clear the loading flag, even on failure, and RECORD
      // the error. Previously this only logged, so a failed fetch left the
      // country field empty, un-tappable and completely silent.
      emit(
        state.copyWith(countriesLoading: false, countriesError: e.toString()),
      );
    }
  }
