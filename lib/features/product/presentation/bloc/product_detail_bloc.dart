...
class ProductDetailBloc extends Bloc<ProductDetailEvent, ProductDetailState> {
  ...
  Future<ProductModel> _fetchProductWithFallback({
    ...
    // Backend supports product(id:) only, not product(urlKey:).
    // Prefer fetching by id; fall back to urlKey only if id is missing.
    ...
    if (normalizedUrlKey.isNotEmpty) {
      return repository.getProductByUrlKey(
        normalizedUrlKey,
        productType: productType,
      );
    }
