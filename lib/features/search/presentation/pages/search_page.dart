...
class _SearchPageViewState extends State<_SearchPageView> {
  ...
  bool _isPriceRangeActive = false;
  Timer? _debounce;
  ...
  void _onSearchChanged(String query) {
    _debounce?.cancel();
    if (query.startsWith('من €') || query.startsWith('price:')) return;
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                _buildSearchModeChips(context, isDark),

                if (_isPriceRangeActive)
                  _buildPriceRangeSelector(context, isDark),
  ...
  Widget _buildSearchModeChips(BuildContext context, bool isDark) {
    final l10n = AppLocalizations.of(context)!;
    return Padding(
        ...
        children: [
          Expanded(
            child: _buildSecondaryButton(
              ...
              onPressed: () => _navigateToImageSearch(context),
            ),
          ),
          ...
          Expanded(
            child: _buildSecondaryButton(
              ...
              label: 'البحث حسب السعر',
              isSelected: _isPriceRangeActive,
              onPressed: () {
                setState(() {
                  _isPriceRangeActive = !_isPriceRangeActive;
                  if (_isPriceRangeActive) {
  ...
  Widget _buildPriceRangeSelector(BuildContext context, bool isDark) {
    return Container(
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.euro, size: 18, color: AppColors.primary500),
                  const SizedBox(width: 6),
                  Text(
                    'تحديد مجال السعر (باليورو €)',
                    style: TextStyle(
                      fontFamily: 'Roboto',
                      fontSize: 14,
          ...
          Row(
            children: [
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  _debounce?.cancel();
                  final min = double.tryParse(_minPriceController.text.trim()) ?? 0.0;
                  final max = double.tryParse(_maxPriceController.text.trim()) ?? 999999.0;
                  _searchController.text = 'من €${min.toStringAsFixed(0)} إلى €${max.toStringAsFixed(0)}';
                  context.read<SearchBloc>().add(SubmitSearch('price:$min-$max'));
                },
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
          ...
          // Rating
          // Rating hidden outside product details page per client request
        ],
      ),
    );
  }
